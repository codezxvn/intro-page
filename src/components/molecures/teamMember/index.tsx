...
export default function TeamMember(props: TeamMemberProps) {
  ...
  return (
    show && (
      <div
        className={`${styles.show} sm:w-1/3 w-full aspect-video lg:aspect-[5/7] sm:aspect-square relative bg-p mb-8`}
            ...
            <div className="flex justify-around items-center w-5/6">
              {props.socialIcons.map((item) => (
                <a
                  key={item.url}
                  href={item.url}
                  className="p-1 rounded-full border-2 border-solid border-primary"
                >
